...
".sky{margin-top:11px;text-align:center}\n"
...
"<div class='card'><div class='ttl'>[ GPS ]</div>\n"
"<div class='gps'>\n"
...
"<div class='sky'><svg viewBox='-100 -100 200 200' preserveAspectRatio='xMidYMid meet'>\n"
...
"var NOMV={p0:12000,p1:5000,p2:2500,p3:3000};\n"
...
"    s.note='Graf ukazuje ODCHYLKU od prumeru okna, ne absolutni hodnotu - v celych Hz by na 10 MHz nebylo videt nic. Zdroj: poll 1 Hz.';\n"
"  } else if(k==='temp'){\n"
"    s.title='TEPLOTY';\n"
...
"    var age=Math.round(n*(1-i/4));\n"
"    u.textContent=(age===0?'ted':('-'+(age<120?age+' s':Math.round(age/60)+' min')));\n"
"    ax.appendChild(u);\n"
"  }\n"
...
"   {n:'VBAT',pct:pctOf(s.vbat_mv,0,3600),mark:pctOf(3000,0,3600),\n"
"    v:s.vbat_mv===null?null:(s.vbat_mv/1000).toFixed(3)+' V',\n"
"    st:(s.vbat_mv!==null&&s.vbat_mv<2600)?'warn':'ok'},\n"
...
"}\n"
"function pollSats(){ fetch('/api/sats').then(function(r){return r.json();}).then(drawSky).catch(function(){}); }\n"
"\n"
...
"    DL=D;\n"
"    $('dlNote').textContent='historie: '+P.length+' bodu z '+(d.total||0)+' zaznamu v logu. '\n"
"      +'Kmitocet null = tehdy nebyl FPGA link. (Napajeni 12/5/VREF a MCU/FPGA teploty datalog neuklada.)';\n"
"    drawAll(); if(zoom) drawZoom();\n"
"  }).catch(function(e){ $('dlNote').textContent='historie se nenacetla: '+e+' - bezi datalog a je log neprazdny?'; DL=null; });\n"
"}\n"
"\n"
