...
| 9 | ⚠️ **IOC dávka: zapnout detekci přetečení zásobníku + zvětšit UartTask** (audit 2026-07-18) | STM32 (**IOC owner**) | **1) `configCHECK_FOR_STACK_OVERFLOW=2` + `configUSE_MALLOC_FAILED_HOOK=1`** — dnes NEJSOU definované, takže FreeRTOS je bere jako 0 a `vApplicationStackOverflowHook`/`vApplicationMallocFailedHook` ve `freertos_hooks.c` se **nikdy nezavolají** (crash black-box „stack:Task" je mrtvý kód, i když je dokumentovaný jako ochrana). **2) UartTask 512→1024 words** — rámec `UartTask_run` je při `-O0` (Debug = co se flashuje) **1256 B z 2048 B zásobníku (61 %)**, zbývá ~790 B na printf + HAL + FPU exception frame; projekt už jedno přetečení zažil. Heap má rezervu (15,4 KB z 32 KB). ⚠️ Obojí je **CubeMX-managed** (`FreeRTOSConfig.h`, `FREERTOS_M7.Tasks01`) → ruční editace zmizí při regeneraci, musí projít přes IOC ownera + `CUBEMX_CHECKLIST.md` (CONTRIBUTING §1). |
