...
| 22 | **Rozdělit D2 SRAM mezi jádra** (linker + `ram` test buffer) | oba | ⬅ nutné **před** předáním D2 jádru CM4 (#24). Dnes má D2 celou CM7: `CM7/STM32H757BITX_FLASH.ld` (i `_RAM.ld`) má `RAM_D2 : ORIGIN = 0x30000000, LENGTH = 288K` a `freertos_task_uart.c` používá `RAM_BASE 0x30000000` pro UART `ram write/read`. ETH deskriptory + lwIP heap půjdou do téže oblasti → **jádra by si tiše přepsala paměť** a vypadalo by to jako náhodná chyba sítě. Rozdělit v obou linker skriptech, test buffer přesunout nebo zrušit. |
