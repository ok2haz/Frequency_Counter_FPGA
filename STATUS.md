...
| 24 | **ETH + lwIP na CM4** (RMII PHY je na desce) | CM4 | ⬅ #20, #22. **W5500 modul NENÍ potřeba** — nativní MAC+PHY (LAN87xx) je osazený, modul by byl krok zpět (~10 Mbit, druhý stack mimo kontrolu, blokuje SPI). Deskriptory a RX buffery v D2 = domácí doména CM4, který **nemá D-cache** → odpadají cache pasti ETH DMA. ⚠️ Otevřené: **typ PHY a jeho adresa** (LAN8720A vs 8742A, strapy MODE/PHYAD ze schématu), pulz `ETH_RES` při initu. |
| 25 | **SCPI server** (TCP 5025 na CM4 + týž parser přes USB CDC na CM7) | oba | ⬅ #24 pro síť; **přes USB jde hned** — CDC konzole už běží (`USE_USB_CDC_CONSOLE=1`), transport je hotový, zbývá napojit parser (`libscpi`). SCPI-99 na raw socketu 5025 = standard laboratorních přístrojů (VISA `TCPIP::…::5025::SOCKET`). Jeden zdroják parseru, dva transporty. GET čte IPC snapshot, SET jde do cmd ringu. Návrh stromu příkazů v `NAVRH…md` §6. ⚠️ Viz zlaté pravidlo — nepouštět ven simulovaná data. |
