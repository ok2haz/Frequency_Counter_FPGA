...
| 25 | 🔶 **SCPI server — JÁDRO + USB HOTOVO 2026-08-07 (hand-rolled), TCP čeká na CM4.** Transportně nezávislé jádro `scpi.c/h`: parser (case-insensitive, krátká/dlouhá forma MEAS↔MEASure, `:` hierarchie, `?` dotazy) + 10 příkazů (`*IDN? *OPC? *RST *CLS`, `SYST:VERS?/ERR?/TEMP?/GPS:STAT?`, `MEAS:FREQ?`, `STAT:OPER:COND?`) vracejících **reálná data** (`g_sensors`/`gps_get`/FPGA driver). ⚠️ Golden rule: `MEAS:FREQ?` = **reálný FPGA** (`fpga_freq_get_last`), NE sim → SCPI NaN `9.91E37` dokud #2. Bez `%f` (celočíselná extrakce). **USB napojeno**: konzolový příkaz `scpi <cmd>` (`freertos_task_uart.c`). Selftest **12/12** (`scpi_selftest`, parser vektory). Zvoleno **hand-rolled** (0 vendored kódu, testovatelné) místo libscpi. **Zbývá:** TCP 5025 ⬅ ETH/CM4 (#24); reálná `MEAS:FREQ?` ⬅ #2; error queue (dnes stub), SET příkazy + arg parsing, rozšíření stromu (CALC:ADEV?/MMEM:DATA?). Návrh stromu v `NAVRH…md` §6/§6a. | oba | jádro+USB hotové; síť ⬅ #24, data ⬅ #2 |
