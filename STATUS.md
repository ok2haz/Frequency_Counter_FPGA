...
| 10 | **Ověřit na HW, že detekce přetečení zásobníku opravdu zabírá** (po opravě #9) | STM32 | Zapnutí je ověřené jen staticky (preprocesor: hook je v `tasks.c` 2×). **Detekce se sama ozve až při skutečném přetečení — dokud ji nevyzkoušíš, nevíš, jestli celý řetěz funguje až do konce.** Postup (~5 min): (1) do libovolného tasku dočasně přidej `volatile char waste[4096]; waste[0] = 1;`, (2) build + flash, (3) po resetu musí System Health / UART `status` ukázat **„Reset: … stack:\<jméno tasku\>"** (crash black-box z BKP_DR3..5) — IWDG resetne do ~4 s, protože hook zastaví heartbeat, (4) **`waste` zase odstranit**. Testuje celý řetěz naráz: hook → `crash_blackbox` → BKP → čtení v `MX_RTC_Init` → zobrazení. ⚠️ Bez VBAT baterie testuj **warm resetem** (BKP nepřežije power-cycle). |
